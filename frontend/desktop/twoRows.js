...
let clickOnSeeOurProducts2 = () => {
    if(styleOfWall) styleOfWall.remove();
    
    whichCatalog = '';
//const { json } = require("body-parser");

//Remove the three rows to replace them by two
if(document.getElementById('products_container')) document.getElementById('products_container').remove
...
let clickOnSeeOurProducts = () => {
    ...
    let i=0;
    for (i; i < products_data.length; i+=/*4*/2) {
        if(products_data[i]){
...
catalogues.addEventListener('click', clickOnSeeOurProducts2);
